# Repository health

Generated by `tools/repo_check.py`. Do not edit by hand.

**Score: 82/100 (grade C).** 0 error(s), 6 warning(s).

This report is a static check. It is not a build, and it does not prove
that the app compiles or that a write reached an ERP.

## Modules

| module | sources | tests | lines |
| --- | --- | --- | --- |
| :app | 38 | 0 | 11595 |
| :data | 8 | 0 | 1173 |
| :design | 10 | 0 | 4267 |
| :domain | 34 | 9 | 6923 |
| :integration | 8 | 5 | 1472 |
| :service | 21 | 8 | 9416 |

Test functions declared: 300.
Room indexes declared: 20; CREATE INDEX statements in the migration: 20.

## Findings

### Warning (6)

- 1277 lines: split this file — `app/src/main/java/app/mizan/feature/home/HistoricalErpActionLogsList.kt`
- 1098 lines: split this file — `service/src/main/kotlin/app/mizan/service/authority/ServiceAuthority.kt`
- 1065 lines: split this file — `app/src/main/java/app/mizan/feature/agent/Agent.kt`
- 989 lines: split this file — `app/src/main/java/app/mizan/feature/account/Account.kt`
- 946 lines: split this file — `service/src/test/kotlin/app/mizan/service/GovernedExecutionTest.kt`
- 875 lines: split this file — `design/src/main/kotlin/app/mizan/design/component/Components.kt`

### Info (4)

- demo credential 'password' in the reference service — `service/src/main/kotlin/app/mizan/service/authority/ServiceAuthority.kt`
- demo credential 'password' in the reference service — `service/src/main/kotlin/app/mizan/service/authority/ServiceAuthority.kt`
- demo credential 'password' in the reference service — `service/src/main/kotlin/app/mizan/service/authority/ServiceAuthority.kt`
- demo credential 'password' in the reference service — `service/src/main/kotlin/app/mizan/service/authority/ServiceAuthority.kt`

