    ...
    fun capabilities(exchange: HttpExchange) = Http.serve(exchange) {
        ...
        Http.respond(
            ...
            Json.obj(
                ...
                // The public halves of the authority's keys. A client build
                // pins these; a deployment that must not be trusted at
                // runtime can ignore this field entirely and ship its pins.
                "receiptAlgorithm" to Json.str(signer?.publicKeys()?.firstOrNull()?.algorithm
                    ?: "HMAC-SHA256"),
                "receiptKeys" to Json.arr(
                    (signer?.publicKeys() ?: emptyList()).map { key ->
                        Json.obj(
                            "keyId" to Json.str(key.keyId),
                            "algorithm" to Json.str(key.algorithm),
                            "publicKey" to Json.str(key.publicKeyBase64),
                            "retired" to Json.bool(key.retired),
                        )
                    },
                ),
            ...
            if (receipt == null) {
                ...
                Http.respond(
                    ...
                    Json.obj(
                        ...
                        // Every field the signature covers is published: a
                        // device that cannot rebuild the signed bytes cannot
                        // verify anything, and a receipt it cannot verify is
                        // only the service's word.
                        "tenantId" to Json.str(receipt.claims.tenantId),
                        "approvalLevel" to Json.str(receipt.claims.approvalLevel.name),
                        "verificationHash" to Json.str(receipt.claims.verificationHash),
                        "traceId" to Json.str(receipt.claims.traceId),
                        ...
                        // The device re-checks this with the pinned public key
                        // rather than believing the "verified" boolean below.
                        "authorityVerified" to Json.bool(verification?.valid == true),
