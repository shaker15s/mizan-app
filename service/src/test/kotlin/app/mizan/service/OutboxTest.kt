...
class OutboxTest {
    ...
    fun aRunningServiceDrainsItsOwnQueue() {
        ...
        try {
            ...
            assertTrue(
                "the sweeper did not drain the queue",
                waitUntil { harness.service.stores!!.outbox.pending().isEmpty() },
            )
            assertNotNull(harness.service.stores!!.journals.get("EXE-OBX-SWEEP"))
        } finally {
            harness.service.stop()
        }
    }
