...
class ReceiptsAndDevicesTest {
    ...
    // ------------------------------------------------- the device's own verdict

    private val authorityKey = AuthorityKeyPair.generate("k-ed25519")

    private fun asymmetricClaims(
        proposalFingerprint: String = "fp-1",
        erpRecordId: String = "SO-1001",
    ) = ReceiptClaims(
        receiptId = "RCT-ED-1",
        executionId = "EXE-ED-1",
        tenantId = "sim-alamal",
        actorId = "USR-REP",
        approverIds = listOf("USR-MGR"),
        ...
        verifiedFields = listOf("amount_total", "partner_id", "state"),
        issuedAtMillis = 1_790_000_000_000L,
        traceId = "TRC-1",
    )

    @Test
    fun aDeviceThatPinsThePublicKeyCanVerifyAReceiptItCannotForge() {
        val signed = ReceiptSigner(listOf(authorityKey)).sign(asymmetricClaims())
        assertEquals(SignatureAlgorithm.ED25519.wire, signed.algorithm)
        val inspection = ReceiptInspector.inspect(
            signed = signed,
            pinnedKeys = ReceiptSigner(listOf(authorityKey)).publicKeys(),
        ...
        assertEquals("RECEIPT_VALID", inspection.reasonCode)
        assertEquals(ReceiptTrust.VERIFIED, inspection.trust)
        assertTrue(inspection.proven)
    }

    @Test
    fun thePublicHalfOfAnAuthorityKeyIsNotThePrivateHalf() {
        val published = ReceiptSigner(listOf(authorityKey)).publicKeys().single()
        assertEquals("k-ed25519", published.keyId)
        assertEquals(SignatureAlgorithm.ED25519.wire, published.algorithm)
        assertFalse(published.publicKeyBase64.contains("PRIVATE"))
        // The HMAC path publishes nothing: there is nothing safe to publish.
        assertTrue(ReceiptSigner(listOf(keyA)).publicKeys().isEmpty())
    }

    @Test
    fun editingOneClaimOfAnAsymmetricReceiptBreaksTheSignature() {
        val signed = ReceiptSigner(listOf(authorityKey)).sign(asymmetricClaims(erpRecordId = "SO-1001"))
        val rewritten = signed.copy(claims = signed.claims.copy(erpRecordId = "SO-9999"))
        val inspection = ReceiptInspector.inspect(
            rewritten,
            ReceiptSigner(listOf(authorityKey)).publicKeys(),
        )
    ...
    @Test
    fun aReceiptSignedWithASecretTheDeviceWouldHaveToHoldIsNotCalledProven() {
        val signed = signer.sign(claims())
        val inspection = ReceiptInspector.inspect(signed, pinnedKeys = emptyList())
    ...
    @Test
    fun aKeyTheDeviceNeverPinnedIsUnknownEvenWhenTheSignatureIsGood() {
        val signed = ReceiptSigner(listOf(authorityKey)).sign(asymmetricClaims())
        val other = AuthorityKeyPair.generate("k-someone-else")
        val inspection = ReceiptInspector.inspect(signed, ReceiptSigner(listOf(other)).publicKeys())
        assertEquals(ReceiptTrust.UNKNOWN_KEY, inspection.trust)
        assertEquals("RECEIPT_KEY_UNKNOWN", inspection.reasonCode)
    }

    @Test
    fun aValidSignatureOverAnotherExecutionIsNotAProofOfThisOne() {
        val signed = ReceiptSigner(listOf(authorityKey)).sign(asymmetricClaims(proposalFingerprint = "fp-1"))
        val inspection = ReceiptInspector.inspect(
            signed,
            ReceiptSigner(listOf(authorityKey)).publicKeys(),
            expected = ReceiptExpectation(tenantId = "sim-alamal", proposalFingerprint = "fp-2"),
        )
        assertEquals(ReceiptTrust.CLAIM_MISMATCH, inspection.trust)
        assertEquals("RECEIPT_CLAIM_FINGERPRINT_MISMATCH", inspection.reasonCode)
        assertFalse(inspection.proven)
    }

    @Test
    fun anotherTenantsReceiptIsNotThisTenantsProof() {
        val signed = ReceiptSigner(listOf(authorityKey)).sign(asymmetricClaims())
        val inspection = ReceiptInspector.inspect(
            signed,
            ReceiptSigner(listOf(authorityKey)).publicKeys(),
            expected = ReceiptExpectation(tenantId = "other-tenant"),
        )
        assertEquals(ReceiptTrust.CLAIM_MISMATCH, inspection.trust)
        assertEquals("RECEIPT_CLAIM_TENANT_MISMATCH", inspection.reasonCode)
    }

    @Test
    fun aRetiredAuthorityKeyStillVerifiesAndIsReportedAsRetired() {
        val retiredAt = Instant.parse("2026-09-01T00:00:00Z")
        val rotated = ReceiptSigner(listOf(authorityKey)).rotate(AuthorityKeyPair.generate("k-new", retiredAt), retiredAt)
        // The receipt was signed by the key that was active at the time.
        val legacy = ReceiptSigner(listOf(authorityKey)).sign(asymmetricClaims())
        val pinned = ReceiptSigner(listOf(authorityKey)).publicKeys()
            .map { it.copy(retired = true) }
        val inspection = ReceiptInspector.inspect(legacy, pinned)
        assertEquals(ReceiptTrust.RETIRED_KEY, inspection.trust)
        assertEquals("RECEIPT_KEY_RETIRED", inspection.reasonCode)
        assertFalse("retired is honest, not proven", inspection.proven)
        assertEquals("k-new", rotated.activeKeyId)
    }

    @Test
    fun anUnreadableSignatureIsMalformedRatherThanValid() {
        val signed = ReceiptSigner(listOf(authorityKey)).sign(asymmetricClaims())
        val garbled = signed.copy(signature = "not base64 at all!!")
        val inspection = ReceiptInspector.inspect(garbled, ReceiptSigner(listOf(authorityKey)).publicKeys())
        assertEquals(ReceiptTrust.MALFORMED, inspection.trust)
        val unsigned = ReceiptInspector.inspect(signed.copy(signature = ""), emptyList())
        assertEquals(ReceiptTrust.UNSIGNED, unsigned.trust)
        assertEquals("RECEIPT_UNSIGNED", unsigned.reasonCode)
    }

    @Test
    fun theAuthorityVerifierAgreesWithItselfAfterRotation() {
        val at = Instant.parse("2026-09-01T00:00:00Z")
        val rotated = ReceiptSigner(listOf(authorityKey)).rotate(AuthorityKeyPair.generate("k-2", at), at)
        val legacy = ReceiptSigner(listOf(authorityKey)).sign(asymmetricClaims())
        assertEquals(ReceiptVerdict.RETIRED_KEY, rotated.verify(legacy).verdict)
        assertEquals(ReceiptVerdict.VALID, rotated.verify(rotated.sign(asymmetricClaims())).verdict)
    }
